...
class Dir():
    ...
    def getPaths(self, rootDir, postfix):
        '''
        ...
        '''
        paths = []
        for fn in glob.glob(rootDir + "/*" + postfix):
            paths.append(fn)
        return paths
    ...
    def categorize_image_by_id(self, image_root, new_root):
        ...
        image_list = self.getPaths(image_root)
        index = 1
    ...
    def find_dir(self, rootDir, count):
        ...
        sub_dirs = self.getNamesFromDir(rootDir)
        for dir in sub_dirs:
            file_paths = self.getPaths(rootDir + '/' + dir)
            file_num = len(file_paths)
            if file_num < count:
                dirs.append(dir)
    ...
    def select_image_single_dir(self, src_root, dst_root, num, postfix):
        self.check_dir(dst_root)

        images = self.getPaths(src_root, postfix=postfix)
        print "total img num: ", len(images)
        if len(images) <= num:
            for img in images:
                self.copyFiles(img, dst_root)
        else:
            for i in range(num):
                self.copyFiles(images[i], dst_root)
        print "Copy Done."


    def select_image(self, src_root, dst_root, num, postfix):
        self.check_dir(dst_root)
        dirs = self.getNamesFromDir(src_root)
        for dir in dirs:
            os.mkdir(dst_root + '/' + dir)
            images = self.getPaths(src_root + '/' + dir, postfix=postfix)
            if len(images) <= num:
                for img in images:
    ...
    def del_specify_dir(self, src_root, dir_name):
        dirs = self.getNamesFromDir(src_root)
        for directory in dirs:
            sub_dir = self.getNamesFromDir(src_root + '/' + directory)
            print sub_dir
            if len(sub_dir) > 0:
                shutil.rmtree(src_root + '/' + directory + '/' + sub_dir[0])
...
def main():
    ...
    # src_root = '/home/hyer/datasets/99image_256x256/444'
    # dst_root = '/home/hyer/datasets/face/intell_99image_merge/merged'
    #
    # ids = dir.getImagePaths(src_root)
    # print ids[0]
    # for id in ids:
    #     path = src_root + '/' + id + '/' + 'pos'
