...
from data import VOCroot, config, logging
...
trained_model = './models/VOC_mobile_300_19105321.pth'
net = build_ssd('test', version, size=300, num_classes=num_classes, bkg_label=0, top_k=200, conf_thresh=conf_thres,
...
dataset = VOCDetection(VOCroot, [(set_type)], BaseTransform(input_size, dataset_mean), AnnotationTransform())
if use_cuda:
    net = net.cuda()
...
def get_voc_results_file_template(image_set, cls):
    ...
    filename = comp_id + 'det_' + image_set + '_%s.txt' % (cls)
    path = os.path.join(eval_result_txt, filename)
    return path
...
def plot_AP_curve(cls_name, rec, prec, ap):
    ...
    plt.savefig(os.path.join(output_dir, cls_name + '_PRCurve_AP=%.4f.png' % ap), format='png')
    plt.close()
...
def evaluate_detections(box_list, output_dir, dataset):
    ...
    do_python_eval(output_dir, use_07=False)
