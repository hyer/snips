...
class Dir():
    ...
    def batch_rename(self, file_root):
        ...
        for directory in dirs:
            files = self.getPaths(file_root + '/' + directory, "*.jpg")
            i = 1
            for file in files:
                os.rename(file, file_root + '/' + directory + '/' + str(i) + '.jpg')
                i += 1

    def batch_rename_dir(self, file_root):
        files = self.getPaths(file_root, "*.jpg")
        i = 1
        for file in files:
            os.rename(file, file_root + '/' + str(i) + '.jpg')
            i += 1
...
def main():
    ...
    dir.batch_rename_dir('/home/hyer/data/k1_scan/JPEGImages')
    print("done")
